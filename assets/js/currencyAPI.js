...
class CurrencyAPI {
    ...
    init(){
        //initialize the db
        this.openDatabase();
        //initialize the UI by populating the currency list and rates
    ...
    //will add the currency list to the db for offline use
    ...
    showCurrenciesFromDatabase() {
        ...
        return this.openDatabase().then( db => {
            ...
            return index.getAll().then( currencies => {
                ...
                //init the ui by populating the rates
                this.queryAPI("USD", "EUR", "United States Dollar equals", "Euro", 1, "amountOne");

            }).catch( error => {
                //matching currency is not foundd
    ...
    getCurrencyList(){
        ...
        fetch('https://free.currencyconverterapi.com/api/v5/currencies').then(response => {
            ...
            this.showCurrenciesFromDatabase(); // get currencies from db offline.
        });

    }

    //add the rates to the rates db offline
    ...
    //ask the user to update the cache
